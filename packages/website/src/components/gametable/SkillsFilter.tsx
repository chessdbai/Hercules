...
export default class SkillsFilter extends React.Component<SkillsFilterProps,SkillsFilterState> {

    constructor(props : SkillsFilterProps) {
        super(props);
        this.state = {
            android: false,
            css: false,
    ...
    doesFilterPass(params: any) {
        ...
        RefData.IT_SKILLS.forEach((skill: string) => {
            if ((this.state as SkillMap)[skill]) {
                if (!rowSkills[skill]) {
                    passed = false;
                }
            }
    ...
    onSkillChanged(skill: string, event: any) {
        const newValue = event.target.checked;
        const newModel : SkillsFilterState = {};
        (newModel as any)[skill] = newValue;
    ...
    render() {
        ...
        const skillsTemplates : React.ReactNode[] = [];
