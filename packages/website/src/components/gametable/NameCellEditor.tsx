...
interface NameCellEditorState {
  putCursorAtEndOnFocus : boolean;
  highlightAllOnFocus : boolean;
  value: number | string;
}

// cell renderer for the proficiency column. this is a very basic cell editor,
export default class NameCellEditor
  extends React.Component<NameCellEditorProps,NameCellEditorState> {

    private putCursorAtEndOnFocus : boolean = false;
    private highlightAllOnFocus : boolean = false;

    constructor(props: NameCellEditorProps) {
        super(props);
    ...
    createInitialState(props: NameCellEditorProps) {
    ...
    isCancelAfterEnd() {
        if (this.state.value && (this.state.value.toString()).toUpperCase() === 'CANCEL') {
            return true;
        } else {
            return false;
        }
