...
interface ProficiencyFilterProps {
  valueGetter: (str: string) => any,
  filterChangedCallback: () => void
}

interface ProficiencyFilterState {
  selected: string
}
...
export default class ProficiencyFilter
  extends React.Component<ProficiencyFilterProps,ProficiencyFilterState> {

    constructor(props: ProficiencyFilterProps) {
        super(props);
        this.state = {
            ...
            rows.push(
                <div key={name} style={{marginTop: 3}}>
                    <label style={{paddingLeft: 4}}>
                        <input type="radio" checked={selected} name={Math.random().toString()}
