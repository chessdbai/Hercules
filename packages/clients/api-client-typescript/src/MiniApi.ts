...
export abstract class MiniApi {
  ...
  protected fetchApiPostRaw = async <TReq, TRes>(resource: string, method: RawMethod, content: Buffer, contentType: string) : Promise<TRes> => {
    const me = this;
    return await retry(async function() {
      return await me.fetchApiRawNoRetry(resource, method, content, contentType);
    }, {
      delay: 200,
      factor: 2,
      maxAttempts: 4,
  ...
  protected fetchApiRawNoRetry = async <TReq, TRes>(resource: string, method: Method, content: Buffer, contentType: string) : Promise<TRes> => {
    try {
      return await this.client.request({
        url: resource,
        method: method,
  ...
  public static requestInterceptor = (req: AxiosRequestConfig) : AxiosRequestConfig => {
    if (req.headers!['Content-Type'].toLower() === 'application/json' && req.data !== undefined) {
  ...
  public static responseInterceptor = (res: AxiosResponse) : AxiosResponse => {
    ...
    var data = res.data;
    if (data !== undefined && res.headers!['content-type'].toLower() === 'application/json') {
